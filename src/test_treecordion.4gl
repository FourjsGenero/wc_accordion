...
public function Test(p_loadMethod string)
  ...
  define
    r_tree wc_treecordion.tTreeCordion,
  ...
  input r_tree.id, r_item.*
    ...
    before input
      call wc_treecordion.Set(r_tree.*)
      call wc_treecordion.Item_Get(r_tree.*, r_tree.id)
        returning r_item.*
    ...
    on action show attribute(text="Show")
      call wc_treecordion.Show(r_tree.*, r_tree.id, TRUE)
    on action hide attribute(text="Hide")
      call wc_treecordion.Show(r_tree.*, r_tree.id, FALSE)
      
    on action expand attribute(text="Expand")
      call wc_treecordion.Expand(r_tree.*, r_tree.id)
    on action expandall attribute(text="Expand All")
      call wc_treecordion.Expand(r_tree.*, "")
      
    on action collapse attribute(text="Collapse")
      call wc_treecordion.Collapse(r_tree.*, r_tree.id)
    on action collapseall attribute(text="Collapse All")
      call wc_treecordion.Collapse(r_tree.*, "")
    ...
    on action visible attribute(text="Is Visible?")
      if wc_treecordion.IsVisible(r_tree.*, r_tree.id)
      then
        message "Visible"
      else
        message "Hidden"
    ...
    on action add attribute(text="Add")
      let r_tree.id = wc_treecordion.Item_Add(r_tree.*, r_tree.id, r_item.*)
    ...
    on action update attribute(text="Update")
      call wc_treecordion.Item_Update(r_tree.*, r_item.*)
    ...
    on action delete attribute(text="Delete")
      let r_tree.id = wc_treecordion.Item_Delete(r_tree.*, r_item.id)
      call wc_treecordion.Item_Get(r_tree.*, r_tree.id)
        returning r_item.*
...
#
function Item_Load(r_tree tTreeCordion)
  ...
  let a_items[1] = NULL
  let p_depth = 2
  let a_items[p_depth] = wc_treecordion.Item_Add(r_tree.*, a_items[p_depth-1], "", "", "Section A", "ActivityMonitor-16.png", "", "", "closed", "This is content for <B>A</B> section")
  let p_depth = 2
  let a_items[p_depth] = wc_treecordion.Item_Add(r_tree.*, a_items[p_depth-1], "", "", "Section B", "Chip1-16.png", "", "", "closed", "<B><li>Item 1</li><li>Item 2</li><li>Item 3</li></B>")
  let p_depth = 3
  let a_items[p_depth] = wc_treecordion.Item_Add(r_tree.*, a_items[p_depth-1], "", "", "Chapter 1", "Dashboard-16.png", "", "", "closed", "This is content for chapter <B>1</B>")
  let a_items[p_depth] = wc_treecordion.Item_Add(r_tree.*, a_items[p_depth-1], "", "", "Chapter 2", "DatabaseActive-16.png", "", "", "closed", "This is content for chapter <B>2</B>")
  let a_items[p_depth] = wc_treecordion.Item_Add(r_tree.*, a_items[p_depth-1], "", "", "Chapter 3", "HardDisk-16.png", "", "", "closed", "This is content for chapter <B>3</B>")
  let p_depth = 2
  let a_items[p_depth] = wc_treecordion.Item_Add(r_tree.*, a_items[p_depth-1], "", "", "Section C", "InternalDrive-16.png", "", "", "closed", "This is content for <B>C</B> section")
  let p_depth = 3
  let a_items[p_depth] = wc_treecordion.Item_Add(r_tree.*, a_items[p_depth-1], "", "", "Chapter 1", "Memory-16.png", "", "", "closed", "This is content for chapter <B>1</B><table>
 <tr>
  <td>Col1<td/>
  <td>Col2<td/>
  <td>Col3<td/>
...
")
  let a_items[p_depth] = wc_treecordion.Item_Add(r_tree.*, a_items[p_depth-1], "", "", "Chapter 2", "network-green-16.png", "", "", "closed", "This is content for chapter <B>2</B>")
  let a_items[p_depth] = wc_treecordion.Item_Add(r_tree.*, a_items[p_depth-1], "", "", "Chapter 3", "network-orange-16.png", "", "", "closed", 'This is content for chapter <B>3</B> which contains an image<img src="http://4js.com/wp-content/uploads/2015/05/logo_4Js_2014_CMYK_seul-300x92.png"/>')
